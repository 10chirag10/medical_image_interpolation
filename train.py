...
validationFlowBackWarp = (model.backWarp(352, 352, device))
...
validationset = dataloader.SuperSloMo(root=args.dataset_root + '/validation', transform=transform, randomCropSize=(352, 352), train=False)
...
def validate():
    ...
    with torch.no_grad():
        for validationIndex, (validationData, validationFrameIndex) in enumerate(validationloader, 0):
            ...
            print("I0 type validate: ",type(I0))
            print("I0 SHAPE validate: ",I0.shape)
            
            flowOut = flowComp(torch.cat((I0, I1), dim=1))
            print("FlowOut shape validate: ",flowOut.shape)
            F_0_1 = flowOut[:,:2,:,:]
            F_1_0 = flowOut[:,2:,:,:]
            print("F_0_1 shape validate: ",F_0_1.shape)
            print("F_1_0 shape validate: ",F_1_0.shape)
            ...
            print("intrpOut validate: ", intrpOut.shape)
            F_t_0_f = intrpOut[:, :2, :, :] + F_t_0
            F_t_1_f = intrpOut[:, 2:4, :, :] + F_t_1
            V_t_0   = F.sigmoid(intrpOut[:, 4:5, :, :])
            V_t_1   = 1 - V_t_0
            print("F_t_0_f shape validate: ",F_t_0_f.shape)
            print("F_t_1_f shape validate: ",F_t_1_f.shape)
            print("V_t_0 shape validate: ",V_t_0.shape)
            print("V_t_1 shape validate: ",V_t_1.shape)
            ...
            # For tensorboard
            if (flag):
#                frame0Temp = frame0[0].numpy() + -25.983
#                frame0Temp = np.swapaxes(frame0Temp,0,1)
#                frame0Temp = np.swapaxes(frame0Temp,1,2)
#                frame0Temp = torch.from_numpy(frame0Temp).type(torch.FloatTensor)
#
#                frameTTemp = frameT[0].numpy() + -25.983
#                frameTTemp = np.swapaxes(frameTTemp,0,1)
#                frameTTemp = np.swapaxes(frameTTemp,1,2)
#                frameTTemp = torch.from_numpy(frameTTemp).type(torch.FloatTensor)
#
#                Ft_pTemp = Ft_p.cpu()[0].numpy() + -25.983
#                Ft_pTemp = np.swapaxes(Ft_pTemp,0,1)
#                Ft_pTemp = np.swapaxes(Ft_pTemp,1,2)
#                Ft_pTemp = torch.from_numpy(Ft_pTemp).type(torch.FloatTensor)
#
#                frame1Temp = frame1[0].numpy() + -25.983
#                frame1Temp = np.swapaxes(frame1Temp,0,1)
#                frame1Temp = np.swapaxes(frame1Temp,1,2)
#                frame1Temp = torch.from_numpy(frame1Temp).type(torch.FloatTensor)
#
#                retImg = torchvision.utils.make_grid([frame0Temp,frameTTemp,Ft_pTemp,frame1Temp], padding=10)
                tempFrame = frame0;
                prinT = revNormalize(tempFrame[0])
...
for epoch in range(dict1['epoch'] + 1, args.epochs):
    ...
    for trainIndex, (trainData, trainFrameIndex) in enumerate(trainloader, 0):
        ...
        print("I0 type: ",type(I0))
        print("I0 SHAPE: ",I0.shape)
        optimizer.zero_grad()
        
        # Calculate flow between reference frames I0 and I1
        flowOut = flowComp(torch.cat((I0, I1), dim=1))
        print("FlowOut shape: ",flowOut.shape)
        # Extracting flows between I0 and I1 - F_0_1 and F_1_0
        F_0_1 = flowOut[:,:2,:,:]
        F_1_0 = flowOut[:,2:,:,:]
        print("F_0_1 shape: ",F_0_1.shape)
        print("F_1_0 shape: ",F_1_0.shape)
        ...
        intrpOut = ArbTimeFlowIntrp(torch.cat((I0, I1, F_0_1, F_1_0, F_t_1, F_t_0, g_I1_F_t_1, g_I0_F_t_0), dim=1))
        print("intrpOut: ", intrpOut.shape)
        # Extract optical flow residuals and visibility maps
        F_t_0_f = intrpOut[:, :2, :, :] + F_t_0
        F_t_1_f = intrpOut[:, 2:4, :, :] + F_t_1
        V_t_0   = F.sigmoid(intrpOut[:, 4:5, :, :])
        V_t_1   = 1 - V_t_0
        print("F_t_0_f shape: ",F_t_0_f.shape)
        print("F_t_1_f shape: ",F_t_1_f.shape)
        print("V_t_0 shape: ",V_t_0.shape)
        print("V_t_1 shape: ",V_t_1.shape)
        # Get intermediate frames from the intermediate flows
    ...
    if ((epoch % args.checkpoint_epoch) == args.checkpoint_epoch - 1):
        ...
        if(checkpoint_counter - 5 >= 0):
            os.remove(args.checkpoint_dir + "/SuperSloMo" + str(checkpoint_counter-5) + ".ckpt")
